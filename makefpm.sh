...
common_opts="--version ${version} --iteration ${iteration} -m arancox@gmail.com --architecture noarch -s cpan --prefix /usr --replaces perl-backup-rdbduprunner --cpan-perl-lib-path /usr/share/perl5 --rpm-summary \"${summary}\" --description \"${description}\""
...
fpm -n $pkg --version ${version} --iteration ${iteration} -m arancox@gmail.com --architecture noarch -C $build_dir -s dir -d perl-Backup-rdbduprunner    -t rpm --rpm-summary "${summary}" --description "${description}" .
fpm -n $pkg --version ${version} --iteration ${iteration} -m arancox@gmail.com --architecture noarch -C $build_dir -s dir -d libbackup-rdbduprunner-perl -t deb --rpm-summary "${summary}" --description "${description}" .
...
fpm --verbose $common_opts $common_deps $rpm_deps -t rpm .
fpm --verbose $common_opts $common_deps $deb_deps -t deb --cpan-package-name-prefix lib --cpan-package-name-postfix -perl .
